...
class TwitchTV {
    ...
    public function sub_length($access_token, $channel, $target) {
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl, CURLOPT_HTTPHEADER, array(
            'Authorization: OAuth ' . $access_token
        ));
        curl_setopt($curl, CURLOPT_URL, $this->base_url.'channels/' .  $channel . '/subscriptions/' . $target);
